...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
	...
	private static double portTxThreshold;
	private static double portRxThreshold;

	private static IOFSwitchService switchService;
	private static IThreadPoolService threadPoolService;
	private static IRestApiService restApiService;
	private static final HashMap<Pair<Match,DatapathId>, FlowBandwith> flowStatistics = new HashMap<>();
	...
	private static ScheduledFuture<?> flowStatisticsCollector;

	private static int flowStatsInterval = 20;
				...
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							log.info("RX: "+String.valueOf((rxBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec));
							log.info("TX: "+String.valueOf((txBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec));
							log.info("Threshold TX: "+String.valueOf(portTxThreshold));
							log.info("Threshold RX: "+String.valueOf(portRxThreshold));

							if (portTxThreshold<(rxBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec ||
									portRxThreshold<(rxBytesCounted.getValue() * BITS_PER_BYTE) / timeDifSec)
							{
								log.warn(npt.getNodeId().getLong()+": Excede el Threshold");
							}
			...
			for (Entry<DatapathId, List<OFStatsReply>> e : replies.entrySet()) {
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFFlowStatsEntry pse : psr.getEntries()) {
						Pair<Match, DatapathId> llave = new Pair<>(pse.getMatch(), e.getKey());
						flowStatistics.put(llave,FlowBandwith.of(
								e.getKey(),
								pse.getByteCount(),
								pse.getPacketCount(),
								pse.getPriority(),
	...
	public void init(FloodlightModuleContext context)
		...
		portRxThreshold= Double.parseDouble(config.get("portRxThreshold"));
		portTxThreshold= Double.parseDouble(config.get("portTxThreshold"));
	...
	private void startStatisticsCollection() {
		...
		flowStatisticsCollector = threadPoolService.getScheduledExecutor().scheduleAtFixedRate(new FlowStatCollector(), flowStatsInterval, flowStatsInterval, TimeUnit.SECONDS);
