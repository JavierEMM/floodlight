...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
	...
	protected void doDropFlow(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		...
		try {
			...
			Ethernet eth =
					IFloodlightProviderService.bcStore.get(cntx, IFloodlightProviderService.CONTEXT_PI_PAYLOAD);			///Validamos si es IPv4
				...
				if (ip.getProtocol().equals(IpProtocol.TCP)) {
					...
					if(tcp.getFlags() == 0x02){
						...
						IPacket ipLayer = new IPv4()
								.setDestinationAddress(ip.getDestinationAddress())
	        					.setSourceAddress(ip.getSourceAddress())
								.setTtl((byte) 128)
								.setPayload(tcpLayer);
